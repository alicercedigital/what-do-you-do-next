...
export const ConflictRoleSchema = z.object({
  id: z.string(),
  name: z.string().describe("Display name for this role (e.g., 'Player', 'Opponent', 'Arena')"),
  description: z.string().describe("Explanation of this role's purpose in the conflict"),
  entityType: RoleEntityTypeSchema.describe("What type of entity can fill this role"),
  required: z.boolean().default(true).describe("Whether this role must be filled for the conflict to start"),
})
...
export const ConflictFormulaTokenSchema = z.object({
  type: z.enum(["role-attribute", "operator", "number", "function", "parenthesis", "comparison", "logical", "string"]),
...
export const ModifyAttributeActionSchema = z.object({
  ...
  operation: z.enum(["set", "add", "subtract", "multiply", "divide"]).default("subtract"),
  formula: z.array(ConflictFormulaTokenSchema).describe("Formula to calculate the value"),
})
...
export const CheckConditionActionSchema = z.object({
  ...
  condition: z.array(ConflictFormulaTokenSchema).describe("Condition formula that evaluates to true/false"),
  thenSteps: z.array(z.string()).describe("Step IDs to execute if condition is true"),
  elseSteps: z.array(z.string()).optional().describe("Step IDs to execute if condition is false"),
})
...
export const LogMessageActionSchema = z.object({
  ...
  template: z.string().describe("Message template with {role.attribute} placeholders"),
})
...
export const ConflictOutcomeSchema = z.object({
  id: z.string(),
  name: z.string().describe("Name of this outcome (e.g., 'Victory', 'Defeat', 'Draw')"),
  description: z.string().describe("What happens when this outcome is triggered"),
  ...
  experienceFormula: z.array(ConflictFormulaTokenSchema).optional().describe("Experience points awarded"),
  itemRewards: z
    .array(
      z.object({
        itemId: z.string(),
    ...
    .default([]),
  triggersGameOver: z.boolean().default(false),
  nextEventId: z.string().optional().describe("Story event to trigger after this outcome"),
})
...
export const RollDiceActionSchema = z.object({
  ...
  modifier: z.array(ConflictFormulaTokenSchema).optional().describe("Bonus to add to the roll"),
})
...
export const CompareAttributesActionSchema = z.object({
  ...
  orderVariable: z.string().optional().describe("Variable to store the ordered list of role IDs"),
})

export type CompareAttributesAction = z.infer<typeof CompareAttributesActionSchema>
...
export const CycleStepSchema = z.object({
  ...
  description: z.string().optional().describe("Explanation of what this step does"),
  action: CycleStepActionSchema,
  // Execution control
  executeFor: z.enum(["once", "each-role"]).default("once").describe("Execute once or for each role of a type"),
  roleTypeFilter: z.string().optional().describe("If executeFor is 'each-role', filter by this role type"),
})
...
export const ConflictEventSchema = z.object({
  id: z.string(),
  name: z.string().describe("Name of this conflict type (e.g., 'Combat', 'Race', 'Exam')"),
  ...
  roles: z.array(ConflictRoleSchema).min(1).describe("Roles that can participate in this conflict"),
  // The cycle of steps that runs each turn
  cycleSteps: z.array(CycleStepSchema).describe("Steps that execute in order each cycle"),
  // Possible outcomes
  outcomes: z.array(ConflictOutcomeSchema).describe("Possible outcomes of this conflict"),
  // Maximum cycles before forced end (safety)
  maxCycles: z.number().default(100).describe("Maximum cycles before forcing an outcome"),
  // Default outcome if max cycles reached
  defaultOutcomeId: z.string().optional().describe("Outcome to trigger if max cycles reached"),
  // UI configuration
  showTurnLog: z.boolean().default(true).describe("Whether to show a log of each turn"),
  animationSpeed: z.enum(["slow", "normal", "fast", "instant"]).default("normal"),
})
...
export const CONFLICT_TEMPLATES: Record<string, Partial<ConflictEvent>> = {
  combat: {
    name: "Combat",
    description: "A battle between characters using physical or magical attacks",
    icon: "swords",
    roles: [
      { id: "player", name: "Player", description: "The player character", entityType: "character", required: true },
      { id: "enemy", name: "Enemy", description: "The opponent in battle", entityType: "character", required: true },
    ],
    outcomes: [
      {
        id: "victory",
  ...
  race: {
    ...
    roles: [
      { id: "player", name: "Player", description: "The player character", entityType: "character", required: true },
      { id: "opponent", name: "Opponent", description: "Racing opponent", entityType: "character", required: true },
      { id: "track", name: "Track", description: "The race location", entityType: "location", required: false },
    ],
    outcomes: [
      {
        id: "first-place",
