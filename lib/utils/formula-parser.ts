 ...
 */
export function applyAttributeCaps(
  value: number,
  minValue?: number,
  maxValue?: number
): number {
...
export function calculateDerivedAttributes(
  ...
  const totalDistributableValues: Record<string, number> = {
    ...baseAttributeValues,
  };
  for (const [attrId, bonus] of Object.entries(equipmentBonuses)) {
    totalDistributableValues[attrId] =
      (totalDistributableValues[attrId] || 0) + bonus;
  }
  ...
  for (const attr of derivedAttrs) {
    if (attr.derivedConfig?.formula && attr.derivedConfig.formula.length > 0) {
      const rawValue = evaluateFormula(
        attr.derivedConfig.formula,
        distributableValues
      );
      result[attr.id] = applyAttributeCaps(
        rawValue,
        attr.derivedConfig.minValue,
        attr.derivedConfig.maxValue
      );
    } else {
      result[attr.id] = 0;
    }
