...
export function executeStep(
  state: ConflictExecutionState,
  step: CycleStep
): ConflictExecutionState {
  const { roleStates, variables, logs } = state;
  const newLogs = [...logs];
  const newRoleStates = [
    ...roleStates.map((r) => ({ ...r, attributes: { ...r.attributes } })),
  ];
  ...
  switch (step.action.type) {
    case "modify-attribute": {
      ...
      if (roleIndex !== -1) {
        const currentValue =
          newRoleStates[roleIndex].attributes[attributeId] ?? 0;
        let newValue: number;
        ...
        newRoleStates[roleIndex].attributes[attributeId] = Math.max(
          0,
          newValue
        ...
        if (operation === "subtract" && value > 0) {
          newLogs.push(
            createLog(
              `${newRoleStates[roleIndex].entityName} loses ${Math.round(
                value
              )} ${attributeId.replace(/_/g, " ")}`,
              "damage"
            )
          );
          ...
          newLogs.push(
            createLog(
              `${newRoleStates[roleIndex].entityName} gains ${Math.round(
                value
              )} ${attributeId.replace(/_/g, " ")}`,
              "success"
            )
    ...
    case "roll-dice": {
      ...
      newLogs.push(
        createLog(
          `Rolled ${diceCount}d${diceSides}: [${rolls.join(", ")}] = ${total}`,
          "info"
        )
      );
      break;
...
export function executeCycle(
  ...
  if (
    ...
    currentState.currentCycle >= state.conflict.maxCycles
  ) {
    const defaultOutcome = state.conflict.outcomes.find(
      (o) => o.id === state.conflict.defaultOutcomeId
    );
    if (defaultOutcome) {
      currentState.isComplete = true;
      currentState.outcome = defaultOutcome;
      currentState.logs.push(
        createLog(`Max cycles reached. ${defaultOutcome.name}`, "info")
      );
    }
  }
...
/**
 * Starts a conflict and returns the initial state
 * This logic was moved from the game store
 */
export function startConflict(
  ...
  character: any
): ConflictExecutionState | null {
  if (!selectedUniverse || !character) return null;

  const conflictEvent = selectedUniverse.conflictEvents?.find(
    (c: any) => c.id === conflictEventId
  );
  if (!conflictEvent) {
  ...
  return createConflictState(conflictEvent, roleAssignments);
}
