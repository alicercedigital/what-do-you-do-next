...
export function CharacterCreator() {
  ...
  const distributableAttributes =
    selectedUniverse?.attributes?.filter(
      (a) => a.category === "distributable"
    ) || [];

  const derivedAttributes =
    selectedUniverse?.attributes?.filter((a) => a.category === "derived") || [];

  const [baseAttributes, setBaseAttributes] = useState<Record<string, number>>(
    () => {
      if (!distributableAttributes.length) return {};
      return Object.fromEntries(
        distributableAttributes.map((attr) => [
          attr.id,
          attr.distributableConfig?.minValue ?? 1,
        ])
      );
    }
  );
  ...
  const usedPoints = Object.values(baseAttributes).reduce(
    (sum, val) => sum + val,
    0
  );
  ...
  const calculatedDerived = useMemo(() => {
    return calculateDerivedAttributes(
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      ...
      >
        <Button
          variant="ghost"
          className="mb-6"
          onClick={() => setCurrentStep("universe-select")}
        >
        ...
        <Card className="bg-card/50 backdrop-blur-sm">
          <CardHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <CardDescription>
                  {selectedUniverse.name} Adventure
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <Progress
                value={(usedPoints / totalPoints) * 100}
                className="mb-6 h-2"
              />
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          attr.display?.iconColor || "text-foreground"
                        )}
                      >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() => updateAttribute(attr.id, 1)}
                          disabled={
                            baseAttributes[attr.id] >= maxValue ||
                            remainingPoints <= 0
                          }
                        >
                          ...
                          {attr.display?.displayType === "bar" && (
                            <div
                              className={cn(
                                ...
                                attr.display.barBackgroundColor ||
                                  "bg-secondary"
                              )}
                            >
                              <div
                                ...
                                style={{
                                  width: `${Math.min(
                                    100,
                                    (value /
                                      (attr.derivedConfig?.maxValue || 100)) *
                                      100
                                  )}%`,
                                }}
                              />
