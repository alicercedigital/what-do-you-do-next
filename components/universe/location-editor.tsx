...
export function LocationEditor({
  ...
  const addAmbient = () => {
    if (newAmbient.trim()) {
      setData({
        ...data,
        ambientSounds: [...data.ambientSounds, newAmbient.trim()],
      });
      setNewAmbient("");
    }
  };
  ...
  const removeAmbient = (index: number) => {
    setData({
      ...data,
      ambientSounds: data.ambientSounds.filter((_, i) => i !== index),
    });
  };
  ...
  return (
    <EditorShell
      title="Location"
      subtitle={location ? `Editing: ${location.name}` : undefined}
      isEditing={!!location}
      onSave={() => onSave(data)}
      onCancel={onCancel}
      canSave={canSave}
      ...
      <div className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setData({ ...data, backgroundImageUrl: e.target.value })
                }
              />
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Track references for music and ambient sounds
            </CardDescription>
          </CardHeader>
