...
export function AttributeEditor({ attribute, allAttributes, universeContext, onSave, onCancel }: AttributeEditorProps) {
  const [data, setData] = useState<GameAttribute>(() => {
    if (attribute) {
      return {
        ...attribute,
        display: { ...DEFAULT_DISPLAY_CONFIG, ...attribute.display },
        distributableConfig: attribute.distributableConfig || DEFAULT_DISTRIBUTABLE_CONFIG,
  ...
  const [activeTab, setActiveTab] = useState<"basic" | "config" | "display">("basic")
  ...
  const addBenchmark = () => {
    ...
    const nextValue = benchmarks.length > 0 ? benchmarks[benchmarks.length - 1].value + 1 : 1
    updateDistributableConfig({
      benchmarks: [...benchmarks, { value: nextValue, label: "", description: "" }],
    })
  }
        ...
        <Card className="mb-6 bg-secondary/30">
          <CardContent className="py-4">
            <div className="flex items-center gap-3">
              <div className={cn("p-2 rounded-lg bg-background", data.display.iconColor)}>
                <Icon className="h-6 w-6" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{data.name || "Attribute Name"}</span>
                  {data.shortName && <span className="text-sm text-muted-foreground">({data.shortName})</span>}
                </div>
                <p className="text-sm text-muted-foreground">{data.summary || "Attribute summary..."}</p>
              </div>
              <div className="text-right">
                <span className="text-xs text-muted-foreground uppercase">
                  {data.category === "distributable" ? "Distributable" : "Derived"}
                </span>
              </div>
            </div>
            ...
            {data.display.displayType === "bar" && (
              <div className="mt-3">
                <div className={cn("h-3 rounded-full", data.display.barBackgroundColor || "bg-secondary")}>
                  <div
                    className={cn("h-full rounded-full transition-all", data.display.barColor || "bg-primary")}
                    style={{ width: "65%" }}
                  />
                </div>
                {data.display.showPercentage && <p className="text-xs text-muted-foreground text-center mt-1">65%</p>}
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
          ...
          <TabsContent value="basic">
            <Card className="py-4">
              <CardHeader>
                ...
                <CardDescription>Define the attribute name, type, and description</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Used in compact UI displays</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={data.category}
                    onValueChange={(v) => updateData({ category: v as "distributable" | "derived" })}
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="distributable">
                        <div className="flex flex-col items-start">
                          ...
                          <span className="text-xs text-muted-foreground">Players assign points (e.g., Strength)</span>
                        </div>
                      </SelectItem>
                      <SelectItem value="derived">
                        <div className="flex flex-col items-start">
                          <span>Derived</span>
                          <span className="text-xs text-muted-foreground">Calculated from formula (e.g., HP)</span>
                        </div>
                      </SelectItem>
                    </SelectContent>
                  </Select>
          ...
          <TabsContent value="config">
            <Card className="py-4">
              <CardHeader>
                <CardTitle>
                  {data.category === "distributable" ? "Distributable Configuration" : "Derived Configuration"}
                </CardTitle>
                <CardDescription>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateDistributableConfig({ minValue: Number.parseInt(e.target.value) || 1 })
                          }
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateDistributableConfig({ maxValue: Number.parseInt(e.target.value) || 10 })
                          }
                        />
                      </div>
                    </div>
                    ...
                    <Collapsible open={showBenchmarks} onOpenChange={setShowBenchmarks}>
                      ...
                      <CollapsibleContent className="mt-3 space-y-3">
                        {(data.distributableConfig?.benchmarks || []).map((benchmark, index) => (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="w-16">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateBenchmark(index, "value", Number.parseInt(e.target.value) || 0)
                                  }
                                  className="mt-1"
                                />
                              </div>
                              ...
                              {(data.distributableConfig?.benchmarks?.length || 0) > 1 && (
                                <Button
                                  type="button"
                                  variant="ghost"
                                  size="icon"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateDerivedConfig({
                              minValue: e.target.value ? Number.parseInt(e.target.value) : undefined,
                            })
                          }
                        />
                        <p className="text-xs text-muted-foreground">Floor value (e.g., HP cannot go below 1)</p>
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateDerivedConfig({
                              maxValue: e.target.value ? Number.parseInt(e.target.value) : undefined,
                            })
                          }
                        />
                        <p className="text-xs text-muted-foreground">Ceiling value (e.g., max HP of 100)</p>
                      </div>
                    </div>

                    {otherAttributes.filter((a) => a.category === "distributable").length === 0 && (
                      <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                        <p className="text-sm text-yellow-600">
                          No distributable attributes available for the formula. Create some distributable attributes
                          first to reference them in derived attribute formulas.
                        </p>
                      </div>
                    )}
                  </>
          ...
          <TabsContent value="display">
            <Card className="py-4">
              <CardHeader>
                ...
                <CardDescription>Configure how this attribute appears in the game UI</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {/* Display Type */}
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => updateDisplay({ displayType: v as "number" | "bar" | "percentage" })}
                  >
                    <SelectTrigger className="bg-transparent">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label>Icon</Label>
                  <IconPicker value={data.display.icon} onChange={(v) => updateDisplay({ icon: v })} />
                </div>
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Display percentage value on the bar</p>
                      </div>
                      <Switch
                        checked={data.display.showPercentage}
                        onCheckedChange={(v) => updateDisplay({ showPercentage: v })}
                      />
                    </div>
                  </>
                )}
                ...
                <div className="border-t pt-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Display this attribute in the character view</p>
                    </div>
                    <Switch
                      checked={data.display.showOnCharacterSheet}
                      onCheckedChange={(v) => updateDisplay({ showOnCharacterSheet: v })}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => updateDisplay({ position: Number.parseInt(e.target.value) || 0 })}
                    />
                    <p className="text-xs text-muted-foreground">Order in the character sheet (lower = first)</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => updateDisplay({ width: v as "full" | "half" | "third" })}
                    >
                      <SelectTrigger className="bg-transparent">
                        <SelectValue />
                      </SelectTrigger>
